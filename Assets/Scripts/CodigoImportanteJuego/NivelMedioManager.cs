...
public class NivelMedioManager : MonoBehaviour
{
    [Header("Paneles")]
    ...
    {
        new int[][] { new int[] { 0, 1, 2,4,8}, new int[] { 0, 1, 4,6 ,8, 9} },
        new int[][] { new int[] { 3,5,6,7 , 8 }, new int[] { 0,1,2,4,5,8,9 } },
        new int[][] { new int[] { 0,1,2, 3,5,6 , 8,9, 11, 12 }, new int[] { 0,1,2,3, 5,6,7,9,10 } }
    };
    ...
    void Start()
    {
        ...
        textoBienvenida.text = "¡Bienvenido al Nivel Medio!";
    ...
    {
        if (isContrarreloj && tiempoRestante <= 0f) tiempoRestante = 1800f;

        while (gameRunning)
        {
            if (isContrarreloj) tiempoRestante -= Time.deltaTime;
    ...
    public void EndGame()
    {
        ...
        int tMin = Mathf.FloorToInt((isContrarreloj ? tiempoRestante : timeElapsed) / 60f);
        int tSeg = Mathf.FloorToInt((isContrarreloj ? tiempoRestante : timeElapsed) % 60f);
        textoTiempoFinal.text = isContrarreloj && tiempoRestante <= 0f ? "Tiempo agotado: 00:00" :
            (isContrarreloj ? $"Tiempo restante: {tMin:00}:{tSeg:00}" : $"Tiempo total: {tMin:00}:{tSeg:00}");

        int iInicio = preguntaIndexSeleccionada * 2;
        if (solucionesImagenes.Length >= iInicio + 2)
        {
            imagenesDeSolucionActuales = new Sprite[] {
                solucionesImagenes[iInicio],
                solucionesImagenes[iInicio + 1]
            };
            imagenActual = 0;
    ...
    {
        if (imagenesDeSolucionActuales == null || imagenesDeSolucionActuales.Length == 0) return;

        imagenActual = (imagenActual + 1) % imagenesDeSolucionActuales.Length;
        MostrarValidacionImagenActual();
    }

    void MostrarValidacionImagenActual()
    {
        ...
        bool casoEspecial = (preguntaIndexSeleccionada == 2 && imagenActual == 0);
        int[] grupoEquivalente = new int[] { 9, 10, 11 };
        int usadosGrupo = 0;
        ...
        for (int i = 0; i < objetosInstanciados.Length; i++)
        {
            ...
            if (casoEspecial && System.Array.Exists(grupoEquivalente, x => x == i))
            {
                if (conectada) usadosGrupo++;
                continue; 
            }
        ...
        if (casoEspecial)
        {
            if (usadosGrupo == 0)
            {
                ...
                foreach (int i in grupoEquivalente)
                {
                    ResaltarErrorVisual(objetosInstanciados[i], Color.yellow);
                }
                errores++;
            }
            else if (usadosGrupo > 1)
            {
